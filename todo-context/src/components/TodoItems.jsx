...
function TodoItems({ todo }) {
    ...
    const editTodo = () => {
        ...
        //sending values to the updateTodo inorder to update them.
    ...
    return (
        <div className={`flex border border-black/10 rounded-lg px-3 py-1.5 gap-x-3 shadow-sm shadow-white/50 duration-300  text-black ${todo.completed ? "bg-[#c6e9a7]" : "bg-[#ccbed7]"}`}>
            ...
            <input
                type="text"
                className={`border outline-none bg-transparent rounded-lg w-full ${isTodoEditable ? "border-black/10 px-2" : "border-transparent"}`}
                value={todoMsg}
